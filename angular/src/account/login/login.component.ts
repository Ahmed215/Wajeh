...
export class LoginComponent extends AppComponentBase {
  ...
  login(): void {
    if (this.isFormValid) {
      if (this.loginService.authenticateModel.phone.startsWith('0')) {
        this.loginService.authenticateModel.phone =
          this.loginService.authenticateModel.phone.substring(1, this.loginService.authenticateModel.phone.length);
      }
      this._tokenAuth
        .authenticateSendSms(this.loginService.authenticateModel)
        .pipe(
          finalize(() => {
          })
        )
