import { Component, OnInit, ChangeDetectionStrategy, ViewChild, Injector, ElementRef, InjectionToken } from '@angular/core';
import { MenuItem, SelectItem } from 'primeng/api';
import { FormBuilder, FormGroup, Validators, AbstractControl } from '@angular/forms';
import { AppComponentBase } from '@shared/app-component-base';
import {
  CompanyDto, AdminRequestServiceProxy, AdminDriverServiceProxy,
  AdminCreateRequestDto, AdminSubategoryServiceProxy, AdminSubcategoryDtoPagedResultDto, AdminOfferPriceDto
} from '@shared/service-proxies/service-proxies';
import { finalize } from 'rxjs/operators';
import { message } from '@shared/Message/message';
import { ActivatedRoute, Router } from '@angular/router';
import { MapsAPILoader, MouseEvent } from '@agm/core';
import { AdminDriverDtoPagedResultDto, UserServiceProxy, AdminCreateRequestWithOfferDto } from '../../../../../shared/service-proxies/service-proxies';
...
export class CreateReservationComponent extends AppComponentBase implements OnInit {
  @ViewChild('Supportingfiles', { static: false }) Supportingfiles;
  @ViewChild('StartPointCtl', { static: false }) public StartPointCtl: ElementRef;
  @ViewChild('endingPointCtrl', { static: false }) public endingPointCtrl: ElementRef;
  ...
  constructor(injector: Injector,
    ...
    private _formBuilder: FormBuilder, private _reservationService: AdminRequestServiceProxy,
    private route: ActivatedRoute, private _router: Router, private _subcategoryService: AdminSubategoryServiceProxy
  ...
  ngOnInit() {
    this.maps.load().then(() => {
      ...
      const SPautocomplete = new google.maps.places.Autocomplete(this.StartPointCtl.nativeElement);
      SPautocomplete.addListener('place_changed', () => {
        this.ngZone.run(() => {
          ...
          this.firstFormGroup.get('startingPoint').setValue(this.latitude + ',' + this.longitude);
          this.getAddress(this.latitude, this.longitude, 'stratingPointTitle');
        });
      });
      const EPautocomplete = new google.maps.places.Autocomplete(this.endingPointCtrl.nativeElement);
      EPautocomplete.addListener('place_changed', () => {
        this.ngZone.run(() => {
          ...
          this.latitude = place.geometry.location.lat();
          this.longitude = place.geometry.location.lng();
          this.firstFormGroup.get('endingPoint').setValue(this.latitude + ',' + this.longitude);
  ...
  deliveryCostChanged ()
  {
    ...
    this.req.net =parseFloat(this.req.vatAmount||0) + parseFloat(this.req.deliveryCost||0) ; 
  }
  ...
  initFirstFormGroup() {
    this.firstFormGroup = this._formBuilder.group({
      ...
      stratingPointAdress: ['', [Validators.required, Validators.maxLength(512)]],
      stratingPointTitle: ['', [Validators.required, Validators.maxLength(512)]],
  ...
  initStates() {
    ...
    this.states.push({ label: this.l('Current'), value: 1 });
    this.states.push({ label: this.l('Schedule'), value: 2 });
    this.states.push({ label: this.l('Previous'), value: 3 });
    this.states.push({ label: this.l('Cancelled'), value: 4 });
  }
  initCatss() {
  ...
  initDrivers(): void {
    ...
    this.driversServices
      .getAll(filterQuery, undefined, undefined, true, '', 0, 1000)
      .pipe(
        finalize(() => {
        })
      )
  ...
  pickupplaceMarker($event) {
    ...
    this.firstFormGroup.get('endingPoint').setValue(this.pickuplatitude + ',' + this.pickuplongitude);
  }
  ...
  pickupmarkerDragEnd($event: MouseEvent) {
    ...
    this.pickupgetAddress(this.pickuplatitude, this.pickuplongitude);
    this.firstFormGroup.get('endingPoint').setValue(this.pickuplatitude + ',' + this.pickuplongitude);
    this.getAddress(this.pickuplatitude, this.pickuplongitude, 'endingPointTitle');
    this.getAddress(this.pickuplatitude, this.pickuplongitude, 'endingPointAdress');
  }
  ...
  onFirstFromSubmit() {
    ...
    if (this.firstFormGroup.valid) {
      ...
      this.reservation.startingPoint = this.firstFormGroup.get('startingPoint').value;
      this.reservation.endingPoint = this.firstFormGroup.get('endingPoint').value;
      this.reservation.stratingPointAdress = this.firstFormGroup.get('stratingPointTitle').value;
      this.reservation.stratingPointTitle = this.firstFormGroup.get('stratingPointTitle').value;
      this.reservation.endingPointTitle = this.firstFormGroup.get('endingPointTitle').value;
      this.reservation.endingPointAdress = this.firstFormGroup.get('endingPointTitle').value;
      this.reservation.subcategoryId = this.firstFormGroup.value.subcategoryId.value;
      this.reservation.deliveryCost = this.firstFormGroup.get('deliveryCost').value;
  ...
  save() {
    ...
    adminOfferPriceDto.driverId = this.firstFormGroup.get('driverId').value.value;
    adminOfferPriceDto.driverName = this.firstFormGroup.get('driverId').value.label;
    adminOfferPriceDto.deliveryCost = this.firstFormGroup.get('deliveryCost').value;
    adminOfferPriceDto.offerStatus = 3;
    adminOfferPriceDto.vat =this.req.VAT;
    model.adminOfferPriceDto = adminOfferPriceDto;
    this._reservationService.createRequestFromAdminPanel(model)
      .pipe(
        finalize(() => {
        })
      )
...
debugger;
    ...
    this.firstFormGroup.get('startingPoint').setValue(this.destinationlatitude + ',' + this.destinationlongitude);
    this.getAddress(this.destinationlatitude, this.destinationlongitude, 'stratingPointTitle');
    this.getAddress(this.destinationlatitude, this.destinationlongitude, 'stratingPointAdress');
  }
  ...
  isUserInRole(roleName: string): boolean {
    let isInRole = false;
    this.userService.getUserRoles(this.appSessionService.user.id, roleName)
    ...
    this.geoCoder.geocode({ 'location': { lat: latitude, lng: longitude } }, (results, status) => {
      if (status === 'OK') {
        if (results[0]) {
          this.zoom = 12;
          this.pickupAddress = results[0].formatted_address;
          this.firstFormGroup.get(ctrlName).setValue(results[0].formatted_address);
        } else {
          window.alert('No results found');
        }
      } else {
  ...
  private setPointAdressToCurrent(is_start : Boolean): void {
    // if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(position => {
        if(is_start)
        {
          this.firstFormGroup.get('startingPoint').setValue( position.coords.latitude + ',' +  position.coords.longitude);
          this.getAddress( position.coords.latitude,  position.coords.longitude, 'stratingPointTitle');
          this.getAddress( position.coords.latitude,  position.coords.longitude, 'stratingPointAdress');
        }
        else
        {
          this.firstFormGroup.get('endingPoint').setValue( position.coords.latitude + ',' +  position.coords.longitude);
          this.getAddress( position.coords.latitude,  position.coords.longitude, 'endingPointTitle');
          this.getAddress( position.coords.latitude,  position.coords.longitude, 'endingPointAdress');
        }
    });
    // }
  }
  ...
  private getDirection() {
    ...
    this.origin = { lat: Number(StartPointArr[0]), lng: Number(StartPointArr[1]) };
    this.destination = { lat: Number(EndingPointArr[0]), lng: Number(EndingPointArr[1]) };
  }
  ...
  getcoords(type,event)
    {
      if(type =='origin')
        {
          this.firstFormGroup.get('startingPoint').setValue( event.lat()+ ',' +  event.lng());
          this.getAddress(event.lat(),  event.lng(), 'stratingPointTitle');
          this.getAddress( event.lat(), event.lng(), 'stratingPointAdress');
        }
        else
        {
          this.firstFormGroup.get('endingPoint').setValue( event.lat() + ',' +  event.lng());
