import {
  AdminCategoryDto, AdminCategoryServiceProxy, AdminSubcategoryDto,
  CompanyServiceProxy, RequestDto, RequestServiceProxy, AdminRequestServiceProxy,
  AdminSubategoryServiceProxy, AdminSubcategoryDtoPagedResultDto, CompanyDtoPagedResultDto, AdminRequestDto, RequestStatusServiceProxy
...
export class ReservationComponent extends AppComponentBase implements OnInit {
  ...
  loadReservations() {
    ...
    this._reservationService
      .getAll(
        (_stat == null || _stat === undefined ||_stat == "") ? undefined : parseInt(filetrObj["state"]),
  ...
  initStates() {
    ...
    .subscribe((result: any) => {
      ...
      statesResult.forEach(element => {
        this.states.push({ label: getCurrentLanguage() === 'en' ? element.nameEn : element.nameAr, value: element.id });
      });
    });
  ...
  initItems() {
    ...
    this.personnelsCols = [
      ...
      { field: 'DeliveryCost', header: this.l('DeliveryCost'),stopFilter:true},
      { field: 'VATAmount', header: this.l('VATAmount'),stopFilter:true},
      { field: 'Net', header: this.l('Net'),stopFilter:true},
      { field: 'CreationTime', header: this.l('CreationTime'),  isCalender:true},
    ];
  }
  ...
  cancelRequest(request) {
    this.message.confirm(this.l('DeleteConfirmation'), this.l('SystemNotification'), (result) => {
      if (result) {
        this._reservationService.cancelRequestFromAdminPanel(request)
          .pipe(
            finalize(() => {
            })
          )
          .subscribe(res => {
            if (!res) {
              this.message.error(this.l('CannotCancel'), this.l('SystemNotification'));
            } else {
              this.message.success(this.l('CancelSuccessfully', request), this.l('SystemNotification'));
              this.loadReservations();
            }
          });
      }
